...
class DPreparedStatement(sql: String) extends PreparedStatement {
  println(s"new DPreparedStatement($sql)")

  override def executeQuery(): ResultSet = {
    println(s"ps.executeQuery()")
    new DResultSet(List(List("hoge", "fuga"), List("piyo", "test")))
  }

  override def setString(parameterIndex: Int, x: String): Unit = {
    println(s"ps.setString($parameterIndex, $x)")
  }

  override def close(): Unit = println("ps.close()")
  ...
  override def unwrap[T](ifase: Class[T]): T = ???

  override def isWrapperFor(ifase: Class[_]): Boolean = ???
