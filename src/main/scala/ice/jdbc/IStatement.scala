...
class IStatement(val connection: IConnection, val statement: Statement) extends Statement with IExtraWorkerHandler {
  ...
  override def executeQuery(sql: String): ResultSet = withExtraWork("executeQuery", Tuple1(sql)) { case Tuple1(sql) => createWrappedResultSet(statement.executeQuery(sql)) }
  ...
  override def getResultSet: ResultSet = withExtraWork("getResultSet") { _ => createWrappedResultSet(statement.getResultSet) }
  ...
  override def getGeneratedKeys: ResultSet = withExtraWork("getGeneratedKeys") { _ => createWrappedResultSet(statement.getGeneratedKeys) }
